...
@Component({
  selector: 'app-show-users-training-end',
  templateUrl: './show-users-training-end.page.html',
  styleUrls: ['./show-users-training-end.page.scss'],
})
export class ShowUsersTrainingEndPage implements OnInit {
      ...
      if (user) {
        ...
        this.storage.get(this.UidAdvanced).then((val) => {
          ...
          for (let i = 0; i < this.storedData.length; i++) {
            ...
            if (this.storedData[i].lessonId == this.PlanId) {
